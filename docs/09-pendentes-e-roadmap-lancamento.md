...
- ~~**R2b** — motor de conformidade + preenchimento no kiosk (online-only) + ação corretiva forçada~~ ✅ Código fechado 2026-07-13 (`docs/R2b-notas.md`): migração `20260713210000` (motor + `obter_checklists_kiosk` + `registar_checklist`), secção Checklists no kiosk, tab Preenchimentos no admin, 22 testes. **Pendente do humano:** `db push` + verificação `verificador-bd` + `gen types --linked`.
